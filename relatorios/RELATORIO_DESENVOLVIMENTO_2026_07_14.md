...
- **Token GitHub**: O token anterior (`ghp_REDACTED_OLD_TOKEN`) expirou/revogou. O token novo (`ghp_REDACTED_OLD_TOKEN`) foi aplicado via `git remote set-url`. Push funciona com refspec explícito `git push origin +HASH:main`.
